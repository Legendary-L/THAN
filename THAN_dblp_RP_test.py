...
def load_data_dblp1(path='/data/wby_data/code/dblp1.mat'):
    data = sio.loadmat(path)
    truelabels, truefeatures = data['label'], data['feature'].astype(float)
    N = truefeatures.shape[0]
    rownetworks = [data['APCPA'] - np.eye(N), data['APA'] - np.eye(N)]  # , data['PTP'] - np.eye(N)]
...
#
#
# def load_data_dblp2(path='/data/wby_data/code/dblp2.mat'):
#     data = sio.loadmat(path)
#     truelabels, truefeatures = data['label'], data['feature'].astype(float)
#     N = truefeatures.shape[0]
#     rownetworks = [data['APCPA'] - np.eye(N), data['APA'] - np.eye(N)]  # , data['PTP'] - np.eye(N)]
...
#
#
# def load_data_dblp3(path='/data/wby_data/code/dblp3.mat'):
#     data = sio.loadmat(path)
#     truelabels, truefeatures = data['label'], data['feature'].astype(float)
#     N = truefeatures.shape[0]
#     rownetworks = [data['APCPA'] - np.eye(N), data['APA'] - np.eye(N)]  # , data['PTP'] - np.eye(N)]
...
label_new=np.full((14475,1),0)

het_walk_f = open("data/dblp/oriData/" + "author_label.txt", "r")
...
tcn.add(tf.keras.layers.LeakyReLU(alpha=0.3))
# 替换最后一层的激活函数
tcn.add(tf.keras.layers.Dense(4, activation='softmax'))
...
# auc_score = roc_auc_score(Y_test2, predict1, multi_class='ovo')
auc_score = roc_auc_score(Y_test2, predict1, multi_class='ovr')
f1_score = f1_score(Y_test2, predict1.argmax(axis=1), average='micro')
