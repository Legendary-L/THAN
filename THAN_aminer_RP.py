...
def load_data_dblp1(path='/data/wby_data/code/aminer1.mat'):
...
def load_data_dblp2(path='/data/wby_data/code/aminer2.mat'):
    data = sio.loadmat(path)
    truelabels, truefeatures = data['label'], data['feature'].astype(float)
    N = truefeatures.shape[0]
    rownetworks = [data['APCPA'] - np.eye(N), data['APA'] - np.eye(N)]  # , data['PTP'] - np.eye(N)]
...
def load_data_dblp3(path='/data/wby_data/code/aminer3.mat'):
    data = sio.loadmat(path)
    truelabels, truefeatures = data['label'], data['feature'].astype(float)
    N = truefeatures.shape[0]
    rownetworks = [data['APCPA'] - np.eye(N), data['APA'] - np.eye(N)]  # , data['PTP'] - np.eye(N)]
...
XX=tf.stack([xx1,xx2,xx3],axis=1)
YY=tf.stack([yy1,yy2,yy3],axis=1)

label_new=np.full((22942,1),0)
...
tcn.add(tf.keras.layers.LeakyReLU(alpha=0.3))
# 替换最后一层的激活函数
tcn.add(tf.keras.layers.Dense(5, activation='softmax'))
